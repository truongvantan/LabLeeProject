...
public interface MemberLabProfileRepository extends JpaRepository<MemberLabProfile, Integer> {
	...
	@Query(value = """
			...
			WHERE
			  ...
			  OR email_public ILIKE CONCAT('%', :keyword, '%')
			  ...
			  OR (to_tsvector('english', coalesce(email_public,'') || ' ' || coalesce(research_interests,'') || ' ' || coalesce(current_organization,'') || ' ' || coalesce(academic_rank,'')) @@ plainto_tsquery('english', :keyword)))
			""", nativeQuery = true)
