...
public class UserController {
	...
	public String listByPage(Model model, @PathVariable(name = "pageNum", required = false) String strPageNum,
		...
		List<Integer> pageNumbers = commonService.getListPageNumbers(totalPageNumber, currentPageNumber);
	...
	public String editUserEnabledStatus(Model model, @PathVariable(name = "userId", required = false) String userId,
		...
		try {
			userService.updateUserStatus(userId, enabled, "enabled");
	...
	@GetMapping("/users/{userId}/deleted/{status}")
	public String editUserDeletedStatus(Model model, @PathVariable(name = "userId", required = false) String userId,
			@PathVariable(name = "status", required = false) boolean deleted, RedirectAttributes redirectAttributes) {
		try {
			userService.updateUserStatus(userId, deleted, "deleted");
			String status = deleted ? "hoàn tác xóa" : "xóa";
			String message = new StringBuffer("").append("Đã ").append(status).append(" người dùng ID ").append(userId)
					.toString();
			redirectAttributes.addFlashAttribute("successMessage", message);
